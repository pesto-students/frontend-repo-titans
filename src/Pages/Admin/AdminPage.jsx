    ...
    <div className="flex flex-col p-8 bg-transparent rounded-lg shadow-lg gym-details lg:flex-row lg:space-x-6">
      ...
      <div className="flex-1">
        ...
        <p className="mb-4 text-gray-300">
          ...
          <span className="text-gray-200">{gym.description}</span>
        </p>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div>
            ...
            <p className="text-gray-200">
              Latitude: {gym.map_detail.coordinates[1]}
            </p>
            <p className="text-gray-200">
              Longitude: {gym.map_detail.coordinates[0]}
            </p>
          </div>
        </div>
        <hr className="my-6 border-gray-600" />
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <p>
              <strong className="text-gray-300">GST:</strong>{" "}
              <span className="text-gray-200">{gym.gst_number}</span>
            </p>
            <p>
              <strong className="text-gray-300">Occupancy:</strong>{" "}
              <span className="text-gray-200">{gym.total_occupancy}</span>
            </p>
          </div>
          <div>
            <p>
              <strong className="text-gray-300">Status:</strong>{" "}
              <span className="text-gray-200">{gym.status}</span>
            </p>
            <p>
              <strong className="text-gray-300">Request Date:</strong>{" "}
              <span className="text-gray-200">
                {moment(gym.req_creation_Date).format("D MMMM YYYY")}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 md:flex-row">
          <div className="flex-1">
            <p>
              ...
              <span className="text-gray-200">{gym.owner_id.email}</span>
            </p>
            <p>
              <strong className="text-gray-300">Phone Number:</strong>{" "}
              <span className="text-gray-200">{gym.owner_id.phone_number}</span>
            </p>
          </div>
          <div className="flex-1">
            <p>
              <strong className="text-gray-300">UPI:</strong>{" "}
              <span className="text-gray-200">{gym.owner_id.upi_id}</span>
            </p>
          </div>
        </div>
        ...
        {/* Reason Textarea */}
        <div className="mb-6">
          <label className="block mb-2 text-gray-300" htmlFor="reason">
            Reason:
          </label>
          <textarea
            id="reason"
        ...
        <div className="flex flex-col items-center justify-between mt-6 space-y-4 lg:flex-row lg:space-y-0">
          ...
          <div className="flex justify-start space-x-4 lg:w-1/2">
            <button
              onClick={handlePrev}
              className="flex items-center px-4 py-2 text-white transition bg-blue-600 rounded-lg hover:bg-blue-500"
            >
          ...
          <div className="flex justify-end space-x-4 lg:w-1/2">
            ...
            <button
              onClick={handleDeny}
              className="flex items-center px-4 py-2 text-white transition bg-red-600 rounded-lg hover:bg-red-500"
            >
...
function AdminPage() {
  ...
  const handleApprove = async () => {
    try {
      const response = await api.patch(`gyms/${gym._id}/response`, {
        status: "approve",
      });
